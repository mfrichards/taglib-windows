...
TagLib_ID3v2Tag *taglib_flac_id3v2(TagLib_File *file)
{
  FLAC::File *f = reinterpret_cast<FLAC::File *>(file);
  return reinterpret_cast<TagLib_ID3v2Tag *>(f->ID3v2Tag(false));
}

TagLib_ID3v2Tag *taglib_wav_id3v2(TagLib_File *file)
{
  RIFF::WAV::File *f = reinterpret_cast<RIFF::WAV::File *>(file);
  return reinterpret_cast<TagLib_ID3v2Tag *>(f->tag());
}

TagLib_ID3v2Tag *taglib_aiff_id3v2(TagLib_File *file)
{
  RIFF::AIFF::File *f = reinterpret_cast<RIFF::AIFF::File *>(file);
  return reinterpret_cast<TagLib_ID3v2Tag *>(f->tag());
}
...
////////////////////////////////////////////////////////////////////////////////
// TagLib::MPEG::XingHeader wrapper
////////////////////////////////////////////////////////////////////////////////
...
TagLib_APETag *taglib_ape_ape(TagLib_File *file)
{
  APE::File *f = reinterpret_cast<APE::File *>(file);
  return reinterpret_cast<TagLib_APETag *>(f->APETag(false));
}
...
UINT taglib_ape_item_binary(const TagLib_APEItem *item, char *buf, UINT max)
{
   const APE::Item *i = reinterpret_cast<const APE::Item *>(item);
   if (i->type() != APE::Item::Binary) return 0;
   ByteVector v = i->value();
...
{
  FLAC::File *f = reinterpret_cast<FLAC::File *>(file);
  return reinterpret_cast<TagLib_XiphTag *>(f->xiphComment(false));
}
...
////////////////////////////////////////////////////////////////////////////////
// TagLib::FLAC::Picture wrapper
////////////////////////////////////////////////////////////////////////////////

UINT taglib_flac_artwork(TagLib_File *file, char *buf, UINT max)
{
   FLAC::File *f = reinterpret_cast<FLAC::File *>(file);
   List<FLAC::Picture *> li = f->pictureList();
   for(List<FLAC::Picture *>::Iterator it = li.begin(); it != li.end(); ++it)
   {
      if ((*it)->type() == 0x03 || (*it)->type() == 0x00) {
         UINT size = (*it)->data().size();
         if (buf == NULL || size > max) return size;
         UINT count = 0;
         for (count = 0; count < size; count++)
         {
           buf[count] = (*it)->data().at(count);
         }
         return count;
      }
   }
